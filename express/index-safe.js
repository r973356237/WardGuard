...
async function startServer() {
  try {
    ...
    // 第三步：尝试初始化定时任务调度器（非阻塞）
    console.log('⏰ 步骤 3/3: 初始化定时任务调度器...');
    try {
      const schedulerService = require('./services/schedulerService');
      await schedulerService.init();
      console.log('✅ 定时任务调度器初始化完成');
    } catch (schedulerError) {
      console.warn('⚠️ 定时任务调度器初始化失败，但服务器将继续启动:', schedulerError.message);
      console.log('💡 邮件提醒功能可能不可用，请检查邮件配置');
    }
    ...
    const gracefulShutdown = (signal) => {
      ...
      try {
        ...
        console.warn('⚠️ 停止定时任务时出错:', error.message);
      }
    ...
    console.error('   5. 运行诊断脚本: node diagnose-and-fix.js');
    process.exit(1);
  }
}
...
startServer().catch(error => {
  ...
  console.error('💡 建议运行诊断脚本: node diagnose-and-fix.js');
  process.exit(1);
});